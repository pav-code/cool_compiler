...
int* Environment_class::return_signature(int Class, int Method)
{
   ...
   while (signature == NULL) {
      ...
      if ( schClass == get_index("Object") ) {
         //TODO: Error cannot find method in pEnv->get_current_class
         cout << "Error: in Class: "<<get_current_class()<<" cannot find Method: " <<Method<<endl;
         break;
      }
   }
...
void program_class::construct_method_environment(ostream& myfile, Environment pEnv, ClassTable* classtable)
{
   ...
   for(int i = classes->first(); classes->more(i); i = classes->next(i)) {
      ...
      if (*f != -1) {
         ...
         classtable->semant_error()<<"Error Method: "<<idtable.lookup(*f)->get_string() \
           <<" in Class: "<<idtable.lookup(C)->get_string()<<" is improperly overloaded (signature must match)."<<endl;
      }
   }	  
}
   ...
   if (signature != NULL) {
     for (int i = 1; i<20; i++) {
         if (args[1]==(-1) && signature[1]==(-1)) {
            ...
            cout << "Error: dispatch called with wrong number of arguments."<<endl;
            break;
         }
     ...
     classtable->semant_error(classtable->get_filename(), this)<< \
       ...
       " cannot find Method: " << name->get_string() <<endl;
   ...
   if (signature != NULL) {
      for (int i = 1; i<20; i++) {
          if (args[1]==(-1) && signature[1]==(-1)) {
             ...
             classtable->semant_error(classtable->get_filename(), this)<< \
              "Error: in Class: "<<idtable.lookup(pEnv->get_current_class())->get_string()<<\
              " - method: " << name->get_string()<<" called with wrong number of arguments (too many, or not enough)." <<endl;
             break;
          }
          check = subclass(args[i], signature[i], pEnv);
             if (!check) {
