...
//Graph_Node used to be here. It's in 'custom-header.h' now due to:
// 1) Environment_class must see it inorder to have a point to InhTree
// 2) this pInhTree will be used to make the classes lub, subclass and
//    get_parent work (used by get_ancestor_list)
...
public:
  ClassTable(Classes);
  void install_basic_classes(Environment );
  void set_filename(Symbol file) { filename=file; }
  Symbol get_filename() {return filename;}

  void print_class(Class_ );
  void print_formals(Formals pFormals, ofstream& myfile);
  ...
  std::map<int, Graph_Node> get_inh_tree();
};
